...
def main():
    ...
    for t in arr:
        t = int(t)
        name = f'PPO1dWindowSizePeriod{t}'
                ...
                if p != t:
                    for num in range(4 - time):
                        if not created:
                            ...
                            PPO.ppo_train(name, '1d', PERIODS[p][0], PERIODS[p][1])
                            created = True
                        else:
                            print(f"Continue training Model {name} with period {p} time {num+1}/{4- time}")
                            ppo_model = PPO.load_ppo_model(name)
                            PPO.ppo_continue_training(name, ppo_model, '1d', PERIODS[p][0], PERIODS[p][1])
                    time = 0

        if not TESTED[t]:
            print(f"Testing model {name} with period {t}")
            ppo_model = PPO.load_ppo_model(name)
            PPO.ppo_test(ppo_model, '1d', PERIODS[t][0], PERIODS[t][1], name)
