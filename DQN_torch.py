...
class DQN:
    ...
    def flatten_state(obs):
        flat_arrays = []
        for _, substate in obs.items():
            if substate.ndim == 1:
                # If the array has one dimension, append it directly
                flat_arrays.append(substate)
            else:
                # If the array has more than one dimension, flatten it
                flat_arrays.append(substate.flatten())
    ...
    def step(self, mode='train'):
        ...
        return action, reward, done, truncated, info
    ...
    def train(self, gamma=0.99, n_episodes=1000, dnn_update_frequency=7, dnn_sync_frequency=30,
        ...
        for episode in range(1, n_episodes + 1):
            ...
            while not done and not truncated:
                ...
                action, reward, done, truncated, info = self.step()
            ...
            if verbose == 1:
                t_train = time.time()
                print("Episode {}\tTotal Reward {:.2f}\tTotal_steps {}\tTime {:.2f} minutes".format(episode,
                                                                                                    self.total_reward,
                                                                                                    step, (
    ...
    def test(self):
        ...
        while not done and not truncated:
            self.state = self.next_state.copy()
            _, reward, done, truncated, info = self.step('test')
            eps_reward += reward
